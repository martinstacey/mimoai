class Click {
  ...
  Click(PVector _pos, PVector _size, String _name, int _type) {
    ...
    name = _name;
    type = _type;
        secondaryFont = createFont("NotoSans-Medium.ttf", 18*scand);
  }
  void calcpos() {
  ...
  void display() {
    ...
    if (type==20||type==21) {                            // //boton redondeado =20 redondeado cuadrado =21 vacio texto adentro
      ...
      fill(240);
      stroke(255);
      strokeWeight(3*scand);
      if (isover()) stroke(240*scand); 
      if (type==20) rect(pos.x, pos.y, size.x, size.y);
      ...
      if (secondaryName!=null) {
        ...
        text(secondaryName, mid.x, mid.y-10*scand);
      }
      
      popStyle();
      
      text(name, mid.x, mid.y+10*scand);
    }
    if (type==30||type==31) {                            //boton icono texto abajo
      ...
      if (type==31) text(name, pos.x-size.x, mid.y-size.y*0.1);
    }
  ...
  void display() {
    ...
    if (type==10||type==11||type==12) {
      ...
      if (!state) {
        ...
        //fill(255,0,0);
        rect(pos.x, pos.y, size.x, size.y); 
        line(mid.x-3*scand,end.y-5*scand,end.x-3*scand,pos.y+3*scand);
        line(mid.x-3*scand,end.y-5*scand,pos.x+3*scand,mid.y);
        //line(pos.x,end.y,end.x,pos.y);
        //fill(255);
        //stroke(coff);
        //ellipse(end.x-(size.y*.5), pos.y+(size.y*.5), size.y*1.05, size.y*1.05);
      }      
      fill(coff);
      if (type==10)  text(name, mid.x-textsize*6, mid.y+7*scand);
      if (type==11)  textAlign(CENTER, CENTER);
      if (type==11)  text(name, mid.x, end.y+textsize*.8);
...
void drawicon(int icon, PVector pos, PVector size) {
  ...
  if (icon==17) { //LOCK CLOSE
    ...
    if (icon==18) { //LEFT
    pushMatrix();
    line(pos.x-size.x*0.1,pos.y-size.y*0.3,pos.x+size.x*0.2,pos.y);
    line(pos.x-size.x*0.1,pos.y+size.y*0.3,pos.x+size.x*0.2,pos.y);
    popMatrix();
    }
        if (icon==19) { //RIGHT
    pushMatrix();
    //strokeWeight(3);
    line(pos.x-size.x*0.0,pos.y-size.y*0.2,pos.x+size.x*0.2,pos.y);
    line(pos.x-size.x*0.0,pos.y+size.y*0.2,pos.x+size.x*0.2,pos.y);
    line(pos.x-size.x*0.3,pos.y,pos.x+size.x*0.2,pos.y);
    //strokeWeight(1);
    popMatrix();
  }
}
