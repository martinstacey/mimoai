...
Click evo;
Option addrooms[];
PVector uipos = new PVector (50, 350);
...
void setupsl(String [] rooms, String [][] roomDATA) {
  ...
  addrooms[0] = new Option(new PVector (width/2-(width/(6*scand)), height/scand-80), new PVector (width/(3*scand), 40), roomcodedeDATAIN(roomDATAIN), 13);
  addrooms[1] = new Option(new PVector (width/2-scand-30, height/scand-40), new PVector (30, 30), rooms, 13);
  addrooms[0].icon = 2;
  addrooms[1].icon = 9;
  for (int l=0; l<addrooms.length; l++) addrooms[l].con = 150;
  for (int l=0; l<addrooms.length; l++) addrooms[l].scaletoandroid(scand);
  evo = new Click(new PVector (width*.5/scand-uisize.x, height/scand-uisize.y*1.5), new PVector (uisize.x, uisize.y), "Go!", 20);
...
void drawsl() {
  ...
  if (ui.page==5) for (int a=0; a<addrooms.length; a++) addrooms[a].display();
  //evo.display();
  //evolveoff();
  //dr.display();
}
